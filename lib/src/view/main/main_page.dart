...
class _MainPageState extends State<MainPage> {
  ...
  Widget _sloganTitle(BuildContext context) {
    final style = MySafarSdk.config.homeHeaderStyle;
    final logoAssetPath = _nonEmpty(style?.logoAssetPath);
    final title = _nonEmpty(style?.title);
    final description = _nonEmpty(style?.description);
    final customText = _composeHeaderText(title, description);
    final text = customText ?? MySafarSdk.brandify("home_slogan".tr());
        ...
        child: Row(
          ...
          children: [
            if (logoAssetPath != null) ...[
              _headerLogo(context, logoAssetPath),
  ...
  String? _composeHeaderText(String? title, String? description) {
    if (title != null && description != null) return "$title - $description";
    return title ?? description;
  }

  /// PNG yoki SVG — extension bo'yicha widget tanlanadi.
  /// Light/dark card fon + radius 16 konteyner ichida.
  Widget _headerLogo(BuildContext context, String path) {
    const double size = 56;
    final bool isDark = context.isDarkMode;
    final style = MySafarSdk.config.homeHeaderStyle;
    final Color bg = style?.logoBackgroundColor ??
        (isDark ? ProjectTheme.cardColorDark : ProjectTheme.cardColorLight);
    final Widget logo = path.toLowerCase().endsWith('.svg')
